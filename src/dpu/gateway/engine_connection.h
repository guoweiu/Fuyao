#ifndef LUMINE_ENGINE_CONNECTION_H
#define LUMINE_ENGINE_CONNECTION_H
...
namespace faas::gateway {
    ...
    public:
        ...
        EngineConnection(Server *server, uint16_t node_id, uint16_t conn_id,
                         gsl::span<const char> initial_data);
        ...
        void SendMessage(const protocol::GatewayMessage &message, gsl::span<const char> payload);

    private:
        enum State {
            kCreated, kRunning, kClosing, kClosed
        };
...
#endif //LUMINE_ENGINE_CONNECTION_H