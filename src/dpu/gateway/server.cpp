...
namespace faas::gateway {
    ...
    void Server::StartInternal() {
        ...
        CHECK(fs_utils::ReadContents(config_file_, &func_config_json))
        << "Failed to read from file " << config_file_;
        CHECK(config_.Load(func_config_json));

        auto gateway_config = config_.GetGatewayConfig();
        CHECK(gateway_config != nullptr);

        // Start IO workers
        auto num_io_worker = gateway_config->num_io_workers;
        CHECK_GT(num_io_worker, 0);
        HLOG(INFO) << fmt::format("Start {} IO workers", num_io_worker);
        for (int i = 0; i < num_io_worker; i++) {
            auto io_worker = CreateIOWorker(fmt::format("IO-{}", i));
            io_workers_.push_back(io_worker);
        }

        auto address = gateway_config->gateway_ip;
        auto engine_conn_port = gateway_config->engine_conn_port;
        auto http_port = gateway_config->http_port;
        auto listen_backlog = gateway_config->listen_backlog;
        auto grpc_port = gateway_config->grpc_port;

        struct sockaddr_in bind_addr{};
        CHECK(!address.empty());
        CHECK_NE(engine_conn_port, -1);
        CHECK_NE(http_port, -1);
        // Listen on address:engine_conn_port for engine connections
        UV_CHECK_OK(uv_ip4_addr(address.c_str(), engine_conn_port, &bind_addr));
    ...
    void Server::OnNewHttpFuncCall(HttpConnection *connection, FuncCallContext *func_call_context) {
        auto func_entry = config_.FindFunctionByFuncName(func_call_context->func_name());
        ...
        DLOG(INFO) << "OnNewHttpFuncCall: " << FuncCallDebugString(func_call);
    ...
    void Server::OnNewGrpcFuncCall(GrpcConnection *connection, FuncCallContext *func_call_context) {
        ...
        FuncCall func_call = NewFuncCallWithMethod(
                ...
                /* client_id= */ 0, next_call_id_.fetch_add(1));
        VLOG(0) << "OnNewGrpcFuncCall: " << FuncCallDebugString(func_call);
        func_call_context->set_func_call(func_call);
        OnNewFuncCallCommon(connection->ref_self(), func_call_context);
    }
    ...
    void Server::DispatchFuncCall(std::shared_ptr<server::ConnectionBase> parent_connection,
        ...
        const FuncCall func_call = func_call_context->func_call();
        ...
        if (engine_connection != nullptr) {
            ...
            DLOG(INFO) << fmt::format("Analyze latency: Gateway will dispatch "
                                     "a function call[func_id={},client_id={},call_id={}] to Engine",
                                     func_call.func_id,
                                     func_call.client_id,
                                     func_call.call_id);
    ...
    bool Server::OnEngineHandshake(uv_tcp_t *uv_handle, gsl::span<const char> data) {
        DCHECK_IN_EVENT_LOOP_THREAD(uv_loop());
        ...
        gsl::span<const char> remaining_data(data.data() + sizeof(GatewayMessage),
    ...
    UV_CONNECTION_CB_FOR_CLASS(Server, HttpConnection) {
        ...
        if (uv_accept(UV_AS_STREAM(&uv_http_handle_), UV_AS_STREAM(client)) == 0) {
            ...
            DLOG(INFO) << "Analyze latency: " << "Gateway accepted a new HTTP connection";
