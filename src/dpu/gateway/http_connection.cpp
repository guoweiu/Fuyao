...
namespace faas::gateway {
    ...
    UV_READ_CB_FOR_CLASS(HttpConnection, RecvData) {
        ...
        if (nread < 0) {
            if (nread == UV_EOF || nread == UV_ECONNRESET) {
                VLOG(0) << "HttpConnection closed by client";
            } else {
    ...
    UV_WRITE_CB_FOR_CLASS(HttpConnection, DataWritten) {
        DLOG(INFO) << "Successfully write response, will resume receiving new data";
        if (status == 0) {
            StartRecvData();
        } else {
    ...
    void HttpConnection::HttpParserOnMessageComplete() {
        StopRecvData();
        DLOG(INFO) << "Start parsing URL: " << std::string(url_buffer_.data(), url_buffer_.length());
    ...
    void HttpConnection::HttpParserOnNewHeader() {
       ...
       // HVLOG(0) << "Parse new HTTP header: " << field << " = " << value;
    ...
    void HttpConnection::OnNewHttpRequest(std::string_view method, std::string_view path,
                                          std::string_view qs) {
        DCHECK_IN_EVENT_LOOP_THREAD(uv_tcp_handle_.loop);
        DLOG(INFO) << "New HTTP request: " << method << " " << path;
        ...
        auto func_entry = server_->GetFuncConfig()->FindFunctionByFuncName(func_name);
        ...
        if (func_entry->qs_as_input) {
            ...
            func_call_context_.append_input(gsl::span<const char>(encoded_json.data(),
                                                                  encoded_json.length()));
        } else {
    ...
    void HttpConnection::SendHttpResponse(HttpStatus status, gsl::span<const char> body) {
        DCHECK_IN_EVENT_LOOP_THREAD(uv_tcp_handle_.loop);
