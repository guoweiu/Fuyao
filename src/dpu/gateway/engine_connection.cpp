...
namespace faas::gateway {
    ...
    void EngineConnection::SendMessage(const GatewayMessage &message, gsl::span<const char> payload) {
        DCHECK_IN_EVENT_LOOP_THREAD(uv_tcp_handle_.loop);
        if (state_ != kRunning) {
    ...
    void EngineConnection::ProcessGatewayMessages() {
        ...
        while (read_buffer_.length() >= sizeof(GatewayMessage)) {
            ...
            if (read_buffer_.length() >= full_size) {
                gsl::span<const char> payload(read_buffer_.data() + sizeof(GatewayMessage),
