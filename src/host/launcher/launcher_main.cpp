...
ABSL_FLAG(std::string, fprocess_output_dir, "",
          "If not empty, stdout and stderr of function processes will be saved in the given directory");
ABSL_FLAG(int, engine_tcp_port, -1, "If set, will connect to engine via localhost TCP socket");
ABSL_FLAG(std::string, device_name, "mlx5_0", "RDMA device name");
ABSL_FLAG(int, device_port, 0, "RDMA device port");
ABSL_FLAG(int, device_gidx, 0, "RDMA device gid index");
...
int main(int argc, char *argv[]) {
    ...
    launcher->set_rdma_device_name(absl::GetFlag(FLAGS_device_name));
    launcher->set_rdma_device_port(absl::GetFlag(FLAGS_device_port));
    launcher->set_rdma_device_gid_index(absl::GetFlag(FLAGS_device_gidx));
