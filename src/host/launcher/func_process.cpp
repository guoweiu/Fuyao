...
namespace faas::launcher {
    ...
    bool FuncProcess::Start(uv_loop_t *uv_loop, utils::BufferPool *read_buffer_pool) {
        ...
        subprocess_.AddEnvVariable("RDMA_DEVICE_NAME", launcher_->rdma_device_name());
        subprocess_.AddEnvVariable("RDMA_DEVICE_PORT", launcher_->rdma_device_port());
        subprocess_.AddEnvVariable("RDMA_DEVICE_GID_INDEX", launcher_->rdma_device_gidx());
    ...
    void FuncProcess::OnSubprocessExit(int exit_status, std::span<const char> stdout,
        ...
        if (stderr.size() > 0) {
            HVLOG(0) << "Stderr: " << std::string_view(stderr.data(), stderr.size());
        }
        state_ = kClosed;
