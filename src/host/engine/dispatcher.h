#ifndef LUMINE_DISPATCHER_H
#define LUMINE_DISPATCHER_H
...
namespace faas::engine {
    ...
    public:
        ...
        bool OnFuncCallCompleted(const protocol::FuncCall &func_call,
                                 int32_t processing_time, int32_t dispatch_delay, size_t output_size, bool is_rdma_request = false);

        bool OnFuncCallFailed(const protocol::FuncCall &func_call, int32_t dispatch_delay);

        bool OnRDMAReclaimMessage(const protocol::MessageTuple &message_tuple);

        void OnRDMAReclaimCompletionMessage(const protocol::FuncCall &func_call);

        void OnRDMARequestCompletionMessage(const protocol::FuncCall &func_call);

    private:
        Engine *engine_;
        uint16_t func_id_;
        const config::FunctionEntry *func_config_entry_;
        size_t min_workers_;
        size_t max_workers_;
        absl::Mutex mu_;
