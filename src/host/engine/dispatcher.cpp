...
namespace faas::engine {
    ...
    bool Dispatcher::OnNewFuncCall(const FuncCall &func_call, const FuncCall &parent_func_call,
        ...
        *dispatch_func_call_message = NewDispatchFuncCallMessage(func_call);
        ...
        if (idle_worker) {
            ...
            DLOG(INFO) << "No idle worker at the moment, func_id = " << func_call.func_id;
    ...
    void Dispatcher::OnRDMARequestCompletionMessage(const protocol::FuncCall &func_call) {

    }

    bool Dispatcher::OnRDMAReclaimMessage(const protocol::MessageTuple &message_tuple) {
        //absl::MutexLock lk(&mu_);
        uint16_t target_client_id = message_tuple.source.client_id;

        //TODO:感觉func_id_不应该和target_client相等
        //DCHECK_EQ(func_id_, target_client_id);
        DCHECK(workers_.contains(target_client_id));

        Message *reclaim_message = message_pool_.Get();
        *reclaim_message = protocol::NewRDMAReclaimMessage(message_tuple);

        if (running_workers_.contains(target_client_id)) {
            LOG(WARNING) << "dispatch reclaim message failed, func_worker is busy";
            return false;
        }

        FuncWorker *func_worker = workers_[target_client_id].get();
        const FuncCall func_call = GetFuncCallFromMessage(*reclaim_message);
        assigned_workers_[func_call.full_call_id] = target_client_id;
        running_workers_[target_client_id] = func_call;
        func_worker->SendMessage(reclaim_message);
        message_pool_.Return(reclaim_message);

        return true;
    }

    bool Dispatcher::OnFuncCallCompleted(const FuncCall &func_call, int32_t processing_time,
                                         int32_t dispatch_delay, size_t output_size, bool is_rdma_request) {
        DCHECK_EQ(func_id_, func_call.func_id);
        ...
        absl::MutexLock lk(&mu_);
        if (!assigned_workers_.contains(func_call.full_call_id) || is_rdma_request) {
            return true;
        }
    ...
    void Dispatcher::OnRDMAReclaimCompletionMessage(const FuncCall &func_call) {
        absl::MutexLock lk(&mu_);
        if (!assigned_workers_.contains(func_call.full_call_id)) {
            LOG(WARNING) << fmt::format("func_call[func_id={}, method_id={}, client_id={},call_id={}] doesn't exist!",
                                        func_call.method_id, func_call.method_id, func_call.client_id, func_call.call_id);
            return;
        }
        uint16_t client_id = assigned_workers_[func_call.full_call_id];
    ...
    bool Dispatcher::OnFuncCallFailed(const FuncCall &func_call, int32_t dispatch_delay) {
        VLOG(0) << "OnFuncCallFailed " << FuncCallDebugString(func_call);
        DCHECK_EQ(func_id_, func_call.func_id);
    ...
    void Dispatcher::DispatchFuncCall(FuncWorker *func_worker, Message *dispatch_func_call_message) {
        ...
        DCHECK(workers_.contains(client_id));
        DCHECK(!running_workers_.contains(client_id));
        const FuncCall func_call = GetFuncCallFromMessage(*dispatch_func_call_message);
    ...
    void Dispatcher::UpdateWorkerLoadStat() {
        ...
        DLOG(INFO) << fmt::format("UpdateWorkerLoadStat: running_workers={}, idle_workers={}",
