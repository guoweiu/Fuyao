#ifndef LUMINE_EVENT_DRIVEN_WOEKER_H
#define LUMINE_EVENT_DRIVEN_WOEKER_H
...
namespace faas::worker_lib {
    ...
    public:
        EventDrivenWorker(std::string debug_file_path = "");

        ~EventDrivenWorker();

        void Start();

        bool is_grpc_service() { return function_config_entry_->is_grpc_service; }

        std::string_view func_name() { return function_config_entry_->func_name; }

        std::string_view grpc_service_name() { return function_config_entry_->grpc_service_name; }
    ...
    private:
        ...
        const config::FunctionEntry *function_config_entry_;
        ...
        inline protocol::FuncCall handle_to_func_call(int64_t handle) {
            protocol::FuncCall func_call;
            memset(&func_call, 0, sizeof(protocol::FuncCall));
...
#endif //LUMINE_EVENT_DRIVEN_WOEKER_H