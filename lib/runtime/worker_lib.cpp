...
namespace faas::worker_lib {
    ...
    using protocol::NewInvokeFuncAcrossHostsMessage;
    ...
    namespace {
        ...
        bool WriteOutputToFifo(const FuncCall &func_call,
                               ...
                               char *pipe_buf) {
            VLOG(0) << "Start writing output to FIFO";
    ...
    // get input and shm region from message
    //if the input is inlined, shm_region is invalid?
    bool GetFuncCallInput(const Message &dispatch_func_call_message,
        ...
        // payload_size<0 means there is no data in payload, the data is in the shared memory
        if (dispatch_func_call_message.payload_size < 0) {
            // Input in shm
            auto input_region = ipc::ShmOpen(ipc::GetFuncCallInputShmName(func_call.full_call_id), false);
            if (input_region == nullptr) {
                LOG(ERROR) << "ShmOpen failed";
                return false;
            }
            //the declaration of input is std::span, but in to_span function, it use gsl::span. Guowei say it is ok, std is written by him, and gsl is written by nightcore
    ...
    bool PrepareNewFuncCall(const FuncCall &func_call, uint64_t parent_func_call,
                            std::span<const char> input,
                            std::unique_ptr<ipc::ShmRegion> *shm_region,
                            Message *invoke_func_message, bool across_hosts) {

        if(across_hosts){
            *invoke_func_message = NewInvokeFuncAcrossHostsMessage(func_call, parent_func_call);
        }else{
